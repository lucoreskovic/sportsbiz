...
• Target: 1–3 picks per day. Three is a CEILING, not a goal.
• Most days produce 1–2 real edges. The rare quiet day produces 1.
• The first pick is the BEST PLAY of the day — your highest conviction, biggest edge.
• A second pick only goes in if it independently clears all three tests.
• A third pick is rare — only when the slate genuinely has three quality spots.
...
3. Least volatile bet type (spreads/totals > moneylines on dogs > player props)

DO NOT round up to a "minimum." DO NOT pad to fill sport categories. DO NOT pick something just because the schedule is full. The schedule being full is irrelevant — what matters is whether 1–3 of those games have a confirmed edge.

If you're staring at the slate trying to find a 3rd pick, the answer is two picks. If you genuinely cannot find a single confirmed edge across the entire slate, return your best researched candidate as 1 medium-confidence pick — but flag honestly that it is the day's best, not a high-conviction play.
...
async function main() {
  ...
  //   • a manual workflow_dispatch (so you can force a fresh run)
  ...
  const shouldRunPicks = isMorningAnchor || isManual || !process.env.GITHUB_ACTIONS;
