...
- posts: 2-3 synthesized fan/industry reaction posts (format below)

### POSTS FIELD — community sentiment samples

Include 2-3 representative reactions that capture how this story is being discussed. These are synthesized — not real quotes — so each is labeled as a "sentiment sample" rather than attributed to a real account.

Each post object:
- handle: a plausible archetype name, e.g. "@CapSpaceAnalyst", "@SaintsBeat_JRobins", "@NFLDraftPundit", "@SalaryCapNerd", "@BruinsFanSince02". Mix of beat reporters, cap/contract analysts, fans, and industry voices.
- body: 1-2 sentences, under 240 chars. Match the handle's archetype — analysts use numbers and cap jargon; beat reporters drop quick-hit facts; fans are opinionated. Vary tone (some supportive, some critical, some neutral-analytical).
- sentiment: "bullish" | "bearish" | "neutral" | "snark" — describes the angle.

Return 2-3 posts per cluster, spanning at least 2 different sentiment types. No emojis beyond a single 🧵 or 📊 if it fits an analyst archetype. No hashtags. No real-person names — archetypes only.

Example set (for a hypothetical "NFL franchise tag" story):
  {"handle":"@CapSpaceAnalyst","body":"Tag value jumps to $32.4M for WRs this year. That's a 9% YoY bump pricing most teams completely out of the market for a second tag.","sentiment":"neutral"}
  {"handle":"@EaglesBeatKline","body":"Howie won't pay tag money for a receiver with DeVonta still on rookie numbers. Not happening.","sentiment":"bearish"}
  {"handle":"@FantasyDynastyAdvice","body":"If tags start flying this late it means extensions stalled. Buy-low window on the tagged guys for dynasty is NOW.","sentiment":"bullish"}
...
${list}`;
  ...
  parsed.clusters = (parsed.clusters||[]).map(c => {
    ...
    // Validate + sanitize the posts array. Drop malformed entries, cap at 3,
    // normalize sentiment. Missing/invalid → empty array (UI hides gracefully).
    const validSentiments = new Set(['bullish','bearish','neutral','snark']);
    const rawPosts = Array.isArray(c.posts) ? c.posts : [];
    const cleanPosts = rawPosts
      .filter(p => p && typeof p.handle === 'string' && typeof p.body === 'string' && p.body.trim().length > 10)
      .slice(0, 3)
      .map(p => ({
        handle: p.handle.trim().slice(0, 40),
        body: p.body.trim().slice(0, 280),
        sentiment: validSentiments.has((p.sentiment||'').toLowerCase()) ? p.sentiment.toLowerCase() : 'neutral',
      }));
    return {
...
Select 10-14 CANDIDATE games where focused research might reveal betting value.

SPORT COVERAGE REQUIREMENT (strict):
• You MUST include at least 1 candidate from soccer (EPL, La Liga, Bundesliga, Serie A, Ligue 1, UCL, or MLS) if ANY soccer games exist in the schedule above.
• Aim for balanced coverage: 2-3 candidates per major sport that has games today. Don't over-index on any one league.
• If a sport has zero games on the schedule, skip it — don't invent matchups.
...
Prioritize answering the question asked. 2-4 facts per candidate is ideal. Be ruthless about flagging NO EDGE when research contradicts the premise — a candidate getting dropped is a win for the reader.`;
  ...
  const ctx = hasResearch
    ...
    : `Real upcoming games from ESPN (web research unavailable this cycle — use your knowledge of these teams):\n${gamesList}`;

  // When research is available we're strict ("drop if no edge"); when it isn't
  // we fall back to knowledge-based picks rather than returning an empty slate.
  const selectionRules = hasResearch
...
• Quality over quantity: 5 well-researched picks > 8 speculative ones. If you have only 4 high-confidence plays, return 4.`
    : `• Web research was unavailable this cycle. Use your sports knowledge to identify 5-8 picks where teams' records, recent form, injuries you know of, or typical matchup patterns suggest value.
• Be honest in the edge field — if a stat isn't verified, write "recent form suggests..." rather than inventing precise numbers. Credibility matters.`;

  const finalPrompt = `You are a sharp sports bettor. Today is ${today}.\n\n${ctx}\n\n` +
`Produce 5-8 final picks. Selection rules:\n${selectionRules}\n` +
`• Include a MIX of bet types: spread, moneyline, total, player_prop, first_half.\n` +
`• Spread sport coverage across US leagues AND soccer — NBA, NFL, MLB, NHL, EPL, La Liga, Bundesliga, Serie A, Ligue 1, UCL, MLS. If a sport has games today, try to include at least one pick from it.\n\n` +
`EDGE FIELD FORMAT: state what justifies the pick. Use concrete numbers and sources when available.\n\n` +
...
`CONFIDENCE: "high" only for picks where evidence is strong. "medium" default. "low" should be rare.\n\n` +
`LEAGUE FIELD: use one of these exact values — NBA, NFL, MLB, NHL, EPL, La Liga, Bundesliga, Serie A, Ligue 1, UCL, MLS.\n\n` +
`Return ONLY valid JSON:\n` +
`{"picks":[{"matchup":"Away @ Home","league":"NBA","type":"spread","when":"Today 7:30 PM ET","pick":"Team +4.5","odds":"-110","edge":"Reason with specific numbers","confidence":"high|medium|low"}]}`;
  ...
  try {
    ...
    // ─── STAGE 4: PER-SPORT BACKSTOP ───────────────────────────────────────
    // For any core sport that has no pick yet AND has games today, run a
    // simpler single-call generation. Resilience guarantee: every sport with
    // games gets at least one pick, even if the research pipeline stumbled.
    const sportLeagues = {
      basketball: ['NBA'],
      football:   ['NFL'],
      baseball:   ['MLB'],
      hockey:     ['NHL'],
      soccer:     ['EPL','La Liga','Bundesliga','Serie A','Ligue 1','UCL','MLS'],
    };
    const leagueToSport = {};
    Object.entries(sportLeagues).forEach(([sport, lgs]) => {
      lgs.forEach(lg => { leagueToSport[lg.toUpperCase()] = sport; });
    });

    const coveredSports = new Set();
    finalPicks.forEach(p => {
      const sport = leagueToSport[(p.league||'').toUpperCase()];
      if (sport) coveredSports.add(sport);
    });

    const sportsWithGamesToday = new Set();
    games.forEach(g => {
      // games entries start with the league label like "NBA: ..." or "LA LIGA: ..."
      const m = g.match(/^([A-Z][A-Z0-9 ]+):/);
      if (m) {
        const sport = leagueToSport[m[1].trim()];
        if (sport) sportsWithGamesToday.add(sport);
      }
    });

    const missingSports = Array.from(sportsWithGamesToday).filter(s => !coveredSports.has(s));
    if (missingSports.length > 0) {
      console.log('[picks] stage 4/4: backstopping missing sports: ' + missingSports.join(', '));
      for (const sport of missingSports) {
        try {
          const allowedLeagues = sportLeagues[sport];
          // Filter the games list down to just this sport
          const sportGames = games.filter(g => {
            const m = g.match(/^([A-Z][A-Z0-9 ]+):/);
            return m && leagueToSport[m[1].trim()] === sport;
          });
          if (sportGames.length === 0) continue;

          const backstopPrompt = `You are a sharp sports bettor. Today is ${today}.

Games available in ${sport.toUpperCase()}:
${sportGames.join('\n')}

Generate 1-2 VALUE picks from these games. Use your knowledge of the teams, recent form, and typical betting markets. Cite specific stats in the edge field (team records, key player status, recent trends).

Rules:
• when field format: "Today H:MM PM ET" or "Tomorrow H:MM PM ET"
• type field: "spread" | "moneyline" | "total" | "player_prop" | "first_half"
• league field must be one of: ${allowedLeagues.join(', ')}
• confidence: "medium" default; "high" only if the edge is strong.

Return ONLY valid JSON:
{"picks":[{"matchup":"Away @ Home","league":"${allowedLeagues[0]}","type":"spread","when":"Today 7:30 PM ET","pick":"Team +4.5","odds":"-110","edge":"Specific reason","confidence":"medium"}]}`;

          const bsRes = await fetch('https://api.anthropic.com/v1/messages', {
            method:'POST',
            headers:{'Content-Type':'application/json','x-api-key':process.env.ANTHROPIC_API_KEY,'anthropic-version':'2023-06-01'},
            body: JSON.stringify({
              model:'claude-haiku-4-5-20251001',
              max_tokens: 1200,
              messages:[{role:'user',content:backstopPrompt}]
            }),
          });
          const bsData = await bsRes.json();
          if (!bsRes.ok) { console.warn('[picks] backstop ' + sport + ' API error'); continue; }
          const bsText = (bsData.content||[]).filter(b=>b.type==='text').map(b=>b.text).join('');
          const bsStart = bsText.indexOf('{'), bsEnd = bsText.lastIndexOf('}');
          if (bsStart === -1) continue;
          const bsRaw = (JSON.parse(bsText.slice(bsStart,bsEnd+1)).picks||[]).filter(p=>p&&p.matchup&&p.pick);
          const bsPicks = bsRaw.map(p => {
            const t = String(p.type||'').toLowerCase().replace(/\s+/g,'_').replace(/prop$/,'player_prop');
            return { ...p, type: validTypes.has(t) ? t : 'spread' };
          });
          console.log('[picks] stage 4/4: backstopped ' + bsPicks.length + ' ' + sport + ' pick(s)');
          finalPicks.push(...bsPicks);
        } catch (e) {
          console.warn('[picks] backstop ' + sport + ' failed:', e.message);
        }
      }
    }
