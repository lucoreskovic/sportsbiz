...
Scan the stories for confirmed injury news involving notable players. Return an "injuries" array. For each injury, provide:
...
Quality bar: only include CONFIRMED injuries (not rumors, not "could miss"). Star-level players preferred over deep bench. Skip if the story is speculative. If no qualifying injuries in today's stories, return injuries: []. Don't fabricate.
      ...
      if (existsSync('stories.json')) {
        ...
        existingPicks = prevPicks.filter(p => {
          ...
          if (nowMs > ts + TWO_HOURS) {
            console.log('[picks] dropping stale pick (game time has passed): ' + p.pick + ' @ ' + p.when);
            return false;
          }
          return true;
        });
