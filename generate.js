...
${list}`;
  ...
  const dominantSources = ['Front Office Sports','Sportico'];
  let dominantCount = 0;

  parsed.clusters = (parsed.clusters||[]).map(c => {
    const indexes = c.storyIndexes || [];
    let leadIdx = indexes[0];
    const leadStory = stories[leadIdx];
    const isDominant = dominantSources.includes(leadStory?.source);

    // If this would be the 3rd+ dominant source cluster, find an alternate source story
    if (isDominant && dominantCount >= 2) {
      const altIdx = indexes.find(i => stories[i] && !dominantSources.includes(stories[i].source));
      if (altIdx !== undefined) leadIdx = altIdx;
    }
    const lead = stories[leadIdx];
    if (lead && !dominantSources.includes(lead.source)) dominantCount = 0;
    else if (isDominant) dominantCount++;

    return {
      category:     c.category     || 'revenue',
      leadHeadline: c.leadHeadline || lead?.title || '',
      leadSource:   `${lead?.source||''} · ${timeAgo(lead?.pubDate)}`,
