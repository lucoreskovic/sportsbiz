...
const YOUTUBE_CHANNELS = [
  { name: 'NBA',  id: 'UCWJ2lWNubArHWmf3FIHbfcQ' },  // NBA official
  { name: 'MLB',  id: 'UCzWQYUVCpZqtN93H8RR44Qw' },  // MLB official
  { name: 'NHL',  id: 'UCqFCMJ17JCBqjR8_9yx9wqg' },  // NHL official
  { name: 'NFL',  id: 'UCDVYQ4Zhbm3S2dlz7P1GBDg' },  // NFL official
  { name: 'ESPN', id: 'UCiWLfSweyRNmLpgEHekhoAg' },  // ESPN
];

// Fallback highlights in case YouTube RSS is unavailable
const FALLBACK_HIGHLIGHTS = [
  { league: 'NBA', title: 'NBA Playoffs Highlights', videoId: 'dQw4w9WgXcQ', thumbnail: 'https://img.youtube.com/vi/dQw4w9WgXcQ/mqdefault.jpg', timeAgo: 'recent', url: 'https://www.youtube.com/c/nba' },
];
...
async function fetchHighlights() {
  ...
  for (const ch of YOUTUBE_CHANNELS) {
    try {
      ...
      console.log(`Fetching YouTube RSS: ${ch.name} (${ch.id})`);
      const res = await fetch(rssUrl, {
        signal: AbortSignal.timeout(10000),
        headers: { 'User-Agent': 'Mozilla/5.0 (compatible; SportsBizNow/1.0)' }
      });
      console.log(`  ${ch.name} RSS status: ${res.status}`);
      if (!res.ok) continue;
      const xml = await res.text();
      console.log(`  ${ch.name} XML length: ${xml.length}`);
      ...
      while ((m = entryRx.exec(xml)) !== null && count < 2) {
        ...
        if (titleMatch && videoIdMatch) {
          ...
          videos.push({
            ...
            videoId:   vid,
            thumbnail: thumbMatch ? thumbMatch[1] : `https://img.youtube.com/vi/${vid}/mqdefault.jpg`,
            timeAgo:   timeLabel,
            url:       `https://www.youtube.com/watch?v=${vid}`,
          });
          count++;
          console.log(`  Added: ${titleMatch[1].trim().slice(0,60)}`);
        }
      }
    } catch(e) { console.warn(`YouTube ${ch.name} failed:`, e.message); }
  }
  console.log(`Total highlights: ${videos.length}`);
  if (videos.length === 0) {
    console.warn('No YouTube highlights fetched — using fallback search links');
    const today = new Date().toLocaleDateString('en-US',{month:'long',day:'numeric',year:'numeric'});
    return [
      { league: 'NBA', title: 'NBA Playoffs Highlights ' + today, videoId: '', thumbnail: 'https://img.youtube.com/vi/mqdefault.jpg', timeAgo: 'latest', url: 'https://www.youtube.com/@NBA/videos', isFallback: true },
      { league: 'MLB', title: 'MLB Highlights ' + today, videoId: '', thumbnail: '', timeAgo: 'latest', url: 'https://www.youtube.com/@MLB/videos', isFallback: true },
      { league: 'NHL', title: 'NHL Playoffs Highlights ' + today, videoId: '', thumbnail: '', timeAgo: 'latest', url: 'https://www.youtube.com/@NHL/videos', isFallback: true },
      { league: 'NFL', title: 'NFL Latest Videos', videoId: '', thumbnail: '', timeAgo: 'latest', url: 'https://www.youtube.com/@NFL/videos', isFallback: true },
    ];
  }
  return videos;
}
