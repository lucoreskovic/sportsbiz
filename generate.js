...
async function generatePicks(headlines) {
  const ctx = headlines.slice(0, 6).join('\n');
  const today = new Date().toLocaleDateString('en-US',{weekday:'long',month:'long',day:'numeric',year:'numeric'});
  const prompt = `You are a sharp sports bettor. Today is ${today}. Based on these sports business headlines, find 3-4 real games being played TODAY or TOMORROW that have genuine betting value. Focus only on today and tomorrow games — no futures, no season-long bets. Look for daily edges: teams on back-to-back schedules, recent roster moves from contract disputes, travel disadvantages, teams fighting for playoff seeding, rivalry motivation, or coaches on the hot seat due to financial pressure.
...
{
  "picks": [
    { "matchup": "Team A vs Team B", "pick": "Team A -3", "odds": "-110", "edge": "one sentence reason tied to news context", "confidence": "high" }
  ]
}

Rules: ONLY games today or tomorrow. No season win totals, no futures. confidence = high, medium, or low. 3-4 picks max. If you cannot identify real games today or tomorrow with genuine value, return an empty picks array.`;
...
async function main() {
  ...
  const headlines = (clustered.clusters||[]).map(cl => cl.leadHeadline);
  clustered.picks = await generatePicks(headlines);
