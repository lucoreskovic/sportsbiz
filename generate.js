...
This is a daily morning post. The picks go up at 7 AM ET and are graded that night. Random picks at -110 lose money long-term — you need 52.4% just to break even, and "fair line, slight lean" picks historically hit ~50%. So a "lean" without a real edge is a losing bet by definition. Don't take it.

PICKS ARE RANKED. Lead with your single highest-conviction play of the day. Then, only if a second and third play also clear the full bar, include them in conviction order.
...
If genuinely no game on the slate clears the bar, return your single best-researched candidate as one medium-confidence pick rather than zero — this is a daily morning post and readers expect at least one play. Empty arrays only when the schedule itself is empty.
...
A day with zero picks is not a failure. It's discipline.`;
  ...
  try {
    ...
    // "feels like"). Long-term, a tight filter > a loose one — random plays at
    // -110 lose money, so a 0-pick day is always better than a coin-flip day.
    ...
    finalPicks = finalPicks.filter(p => {
      ...
      // Demand at least 2 distinct numbers (a stat needs a value AND a context —
      // "12 ppg in last 8 games" has 2 numbers; "team is good" has 0).
      const numCount = (edge.match(/\d+(?:\.\d+)?/g) || []).length;
      if (numCount < 2) {
        console.log('[picks] DROP (only ' + numCount + ' numbers — too thin): ' + p.pick);
        return false;
      }
      // Require some indicator the edge is sourced (a beat reporter, a publication,
      // an "official" / "confirmed" / "reported" marker, or a date reference).
      ...
      if (!hasSourceMarker) {
        console.log('[picks] DROP (no sourcing language in edge): ' + p.pick);
        return false;
      }
      // Hedge-phrase scan. If the model snuck in a "lean" or "fair line", it told
      // us itself this isn't a real pick — drop it.
      const hedgeHit = HEDGE_PHRASES.find(rx => rx.test(edge));
      if (hedgeHit) {
        console.log('[picks] DROP (hedge phrase): ' + p.pick);
        return false;
      }
      return true;
    });
