...
'Every pick must cite specific verifiable facts in the edge field: ATS record, SU record, injury status, rest situation, home/away splits, recent player per-game averages, opponent defensive ranks, starting pitchers for MLB, weather for outdoor games.\n\n' +
'TIME FORMAT: the "when" field MUST be "Today H:MM PM ET" or "Tomorrow H:MM PM ET" (e.g. "Today 7:30 PM ET", "Tomorrow 8:00 PM ET"). No vague labels like "Tonight" or "Game 3" in this field. Put playoff game numbers in the matchup field, e.g. "Ottawa Senators @ Carolina Hurricanes (Game 3)".\n\n' +
'TYPE FIELD: exactly one of: "spread" | "moneyline" | "total" | "player_prop" | "first_half".\n\n' +
'PICK FIELD format by type:\n' +
'  spread      → "Team +/-X.X"                    e.g. "Knicks -5.5"\n' +
'  moneyline   → "Team ML"                        e.g. "Thunder ML"\n' +
'  total       → "Over/Under X.X"                 e.g. "Under 8.5"\n' +
'  player_prop → "Player Over/Under X.X Stat"     e.g. "Jalen Brunson Over 28.5 Points"\n' +
'  first_half  → "Team +/-X.X (1H)"               e.g. "Celtics -3.5 (1H)"\n\n' +
'Return ONLY valid JSON:\n' +
'{"picks":[{"matchup":"Away @ Home","league":"NBA|NFL|MLB|NHL","type":"spread","when":"Today 7:30 PM ET","pick":"Team +4.5","odds":"-110","edge":"Specific reason with real data","confidence":"high|medium|low"}]}';
...
async function fetchHighlights() {
  ...
  const CHANNELS = [
    ...
    {name:'MLS', id:'UCSZbXT5TLLW_i-5W8FZpFsg'},  // MLS
  ];
