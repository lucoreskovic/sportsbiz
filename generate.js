  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    res = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        // max_tokens dropped from 14000 → 9000. Actual outputs cap around
        // 6-9k; the extra headroom was unused but still billed against the
        // per-minute output token rate limit.
        max_tokens: 9000,
  ...
  try {
    ...
    const res = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        model: 'claude-haiku-4-5-20251001',
        max_tokens: 4000,
        // max_uses dropped from 15 → 5. With ~6-8 clusters and most headlines
        // surfacing 0 tweets anyway, 5 searches is plenty to cover the slate.
...
Prioritize answering the question asked. 2-4 facts per candidate is ideal. Be ruthless about flagging NO EDGE when research contradicts the premise — a candidate getting dropped is a win for the reader.`;
    ...
    try {
      ...
      const researchRes = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({
          model: 'claude-haiku-4-5-20251001',
          max_tokens: 8000,
          // max_uses dropped from 20 → 6. Each web_search pulls full page contents
          // into input tokens; the first 2-3 hits per question carry the signal,
          // the rest were burning credits without changing the briefing.
  ...
  const finalPrompt = `You are a professional sports analyst. Today is ${today}.

${ctx}

YOUR JOB: PRODUCE 2-4 WELL-RESEARCHED PICKS PER DAY ON REAL BOOKMAKER LINES.

This is a sports business site. Picks are a content feature — readers want substantive, researched analysis on real games against real lines. Not every pick needs to beat the market by a huge margin. What every pick needs is: a real game, a real line, and current factual analysis the reader can trust.

═══════════════════════════════════════════════════════════════
WHAT'S REQUIRED ON EVERY PICK
═══════════════════════════════════════════════════════════════

1. REAL GAME WITH REAL LINE
   The market line is shown above for every game with the actual book offering it. Use that exact line. Never invent a line. The pick text must match the line shown — if the schedule says "Nuggets -7.5 (DraftKings)", the pick is "Nuggets -7.5", not "-8" or "-6".

2. CURRENT FACTUAL RESEARCH
   The edge field must cite at least ONE concrete, verifiable fact from the research briefings: a stat with a source, an injury report, a lineup change, a scheduling spot, recent form numbers, or a relevant trend. Not generic prose. Specific numbers and sources.

3. HONEST CONFIDENCE LABEL
   - "high" = strong factor + research confirms + line price looks slightly favorable
   - "medium" = solid factor + research backs the lean + line is roughly fair
   - Do NOT use "low" — if the pick is that weak, drop it
   The reader trusts the label, so calibrate honestly. Most picks should be "medium." "High" means you'd actually take this bet yourself.

═══════════════════════════════════════════════════════════════
HOW MANY PICKS
═══════════════════════════════════════════════════════════════

• Default target: 2-4 picks per day
• MAX 5 picks
• MIN 2 picks (when there are real games on the schedule)
• If the schedule is genuinely empty (rare day, no major sports), 1 is acceptable

If you find yourself wanting to return 0 or 1 picks because "nothing has clear edge," that's fine for an internal model but wrong for this product. Your job is to pick the 2-4 most interesting researched plays from today's slate. Even on quiet days, there are games worth analyzing.

You do NOT need to claim every pick has +EV. Some picks are "the line is fair, here's why I lean this side." That's an honest medium-confidence pick. Label it as such.

═══════════════════════════════════════════════════════════════
EDGE FIELD FORMAT
═══════════════════════════════════════════════════════════════

Structure the edge as 2-3 short paragraphs (not bullet points). Make it readable. Example:

"Devin Booker is OUT (Shams, 2hr ago). Phoenix has averaged 14.2 fewer points per game without him in an 8-game sample this season. Their offense ranks 22nd over that stretch.

Denver enters with 3 days rest while Phoenix is on the second night of a back-to-back, traveling from LA. The Nuggets cover the spread 67% of the time at home off rest advantage.

Line currently at -7.5 on DraftKings. Lean Nuggets. Medium confidence — Booker absence is real, but Phoenix has been one-man teams before and Durant can carry stretches."

Three things this example does right:
- Concrete numbers and sources (Shams, the 14.2 ppg, the 67% cover rate)
- Acknowledges the actual line and book
- Honest about the level of confidence — doesn't oversell

═══════════════════════════════════════════════════════════════
WHAT TO AVOID
═══════════════════════════════════════════════════════════════

• Generic prose: "Better team should win at home." That's not analysis.
• Inventing stats: don't quote "team scores 116 ppg in their last 10" if the briefing didn't say so.
• Inventing lines: never write a line different from what the schedule shows.
• Hedging: "could go either way" or "tough one to call" — if it's that uncertain, pick a different game.
• Hype framing: "LOCK OF THE DAY" or "must-bet" language. Stay analytical.
...
ODDS: copy the price from the schedule (e.g., "-110", "+145"). Don't invent.
...
A day with zero picks is not a failure. It's discipline.`;
  ...
  try {
    ...
    // ─── DISCIPLINE FILTER ─────────────────────────────────────────────────────
    // Light quality gate: only drop clearly bad picks (low confidence, missing
    // edge text, no concrete numbers anywhere). Don't penalize a pick just for
    // using different prose phrasing — the prompt + research stage already
    // enforced the structural requirements.
...
async function main() {
  ...
  // 12:00 UTC = 8 AM ET during DST; 13:00 UTC = 8 AM ET after DST ends in Nov.
  // Accept both so we don't have to edit this when clocks change.
  const isMorningAnchor = nowUtcHour === 12 || nowUtcHour === 13;
  ...
  if (shouldRunPicks) {
    ...
    try {
      if (existsSync('stories.json')) {
        ...
        existingPicks = Array.isArray(prev.picks) ? prev.picks : [];
      }
    } catch (e) {
      console.warn('Could not read existing picks from stories.json:', e.message);
    }
    clustered.picks = existingPicks;
    console.log('Skipped picks generation (off-cycle run); preserved ' + existingPicks.length + ' existing picks');
  }
