...
async function fetchHighlights() {
  ...
  for (const ch of CHANNELS) {
    try {
      ...
      const titleMatch = firstEntry > -1 ? /<title>([^<]+)<\/title>/.exec(xml.slice(firstEntry)) : null;
      const pubMatch   = firstEntry > -1 ? /<published>([^<]+)<\/published>/.exec(xml.slice(firstEntry)) : null;
      if (vidMatch) {
        const vid = vidMatch[1].trim();
        const rawTitle = titleMatch ? titleMatch[1] : ch.name + ' Highlights';
        ...
        console.log('Got video for ' + ch.name + ': ' + vid);
      } else {
        console.warn('No video ID in RSS for ' + ch.name);
      }
