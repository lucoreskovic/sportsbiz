...
- summary: 2 sentences, under 180 chars — tone guide applies. MUST be about the specific event in leadHeadline, not the broader topic.
- article: 220-300 words. **MUST be ABOUT the specific event named in leadHeadline.** Lead the article with the specific facts of that event (who, what, when, the actual numbers reported). The article is allowed to add analytical context — implications, comparable past deals, who wins/loses — AFTER establishing the specific story facts. Do not write a generic thematic piece that merely uses the headline as a jumping-off point. If the headline says "Netflix announces expanded NFL deal," the article must lead with what Netflix actually announced (number of games, year, terms, source) before zooming out. tone guide applies.
...
{"byIndex":{"0":["https://twitter.com/...","https://twitter.com/..."],"1":[],"2":["https://x.com/..."]}}`;
  ...
  const TWEET_MAX_AGE_DAYS = 3;
  const maxAgeMs = TWEET_MAX_AGE_DAYS * 24 * 60 * 60 * 1000;
  function tweetIsFresh(oembed) {
    const m = String(oembed.html || '').match(/>([A-Z][a-z]+ \d{1,2}, \d{4})<\/a>/);
    if (!m) return true; // no parseable date — keep, the prompt gate already screened it
...
**IMPORTANT: TODAY IS DEGRADED MODE.** Live bookmaker lines are temporarily unavailable. Do NOT return an empty picks array because of missing lines. Instead, evaluate each researched candidate based on the strength of its named edge (injury severity, scheduling spot, statistical mismatch) and pick the 1-2 candidates with the strongest verified edges from the research briefings below. Mark them medium confidence and note in the edge text that lines weren't available for cross-verification. Return ZERO board-tier picks today — the board requires real lines.
...
YOUR JOB: PRODUCE A TWO-TIER DAILY SLATE.

TIER 1 — EDGE PICKS (1-3): the best researched plays of the day. These carry the record. Full bar applies (all three tests below).
TIER 2 — VALUE BOARD (per sport): additional plays taken straight from the real bookmaker lines in the schedule, so every sport with games gets coverage. Lighter bar, honest confidence labels (low is allowed here), shorter writeups.
...
TIER 1 — EDGE PICKS ("tier":"edge"):
• 1–3 picks. Each must clear ALL THREE tests above. Confidence "high" or "medium" only.
• The first edge pick is the BEST PLAY of the day — your highest conviction.
• Player props are FIRST-CLASS edge picks, not consolation prizes. A clear stat-vs-defense mismatch IS an edge.
• DO NOT round up to 3. A 1-edge day is fine.

TIER 2 — VALUE BOARD ("tier":"board"):
• For EVERY sport that has games in the lines list, add 1–3 board picks (in addition to any edge picks in that sport).
• Board picks come ONLY from games shown in the lines list. Copy the matchup, line, and odds EXACTLY as shown — never invent or adjust a number.
• The bar is lighter: a real angle (rest advantage, road/home split, pace matchup, motivation spot, line-shopping value where one book is clearly off the consensus) stated honestly. You do NOT need breaking news.
• Confidence must be honest: "medium" for a solid angle, "low" for a defensible lean. Most board picks should be "low" or "medium" — that's the point of the tier.
• Edge text for board picks: ONE short paragraph (2-3 sentences) naming the angle and the price. No padding.
• Do not duplicate an edge pick's game AND side on the board. Different side or different market of the same game is fine.
• If a sport has games but genuinely nothing playable, 1 board pick on its single most defensible angle is still required — label it "low" and say why it's thin.
• If the lines list is empty (degraded mode), return ZERO board picks. Board picks without real lines are forbidden.

PICK ORDER: all edge picks first (conviction order), then board picks grouped by sport.

TIE-BREAKER WHEN CHOOSING THE BEST EDGE PICK:
1. Largest verified edge (most concrete supporting facts)
2. Most favorable line (sharpest priced wrong)
3. Most reliable bet type for the named edge (spread/total for game-line edges, player_prop for stat-trend edges)

DO NOT pad the EDGE tier — that's what the board is for. A medium-confidence researched candidate is better than zero edge picks; empty days lose user trust.

If you genuinely cannot find ANY confirmed edge across the entire slate (truly rare — All-Star break with one game on TV), return your best researched candidate as 1 medium-confidence edge pick.

═══════════════════════════════════════════════════════════════
CONFIDENCE LABELS
═══════════════════════════════════════════════════════════════

  • "high"   = multiple supporting facts, line clearly hasn't adjusted, you'd bet 2 units. EDGE tier only, and rare.
  • "medium" = one strong fact, line offers fair-to-slightly-good price, you'd bet 1 unit.
  • "low"    = a defensible angle and a fair price — BOARD tier only. An edge pick can never be "low": if an edge pick feels low-confidence, it's not an edge pick. Move it to the board or drop it.

If most edge picks are "medium" and most board picks are "low", that's correct and honest.
...
• Generic narrative: "Team A is the better team and should win at home." Drop (board tier: only acceptable if backed by a concrete number from the lines list or schedule, like the record differential or rest days).
• Stats with no source: "they cover 67% of the time in this spot" without the research briefing backing it. Drop.
• Hedge language in EDGE picks: "lean," "small edge," "could go either way," "feels like." If you wrote those words in an edge pick, it belongs on the board or in the trash. (Board picks may honestly say "lean" — that's what "low" confidence means.)
• Sport quotas in the EDGE tier: "I should add an NHL edge pick." No — sport coverage is the board's job.
• "Fair line, here's the side I prefer" as an EDGE pick. This is the exact profile of a -110 coin flip and it is the reason records like 9-14 happen. Board it or drop it.
• Inventing or rounding a line/price ANYWHERE. Every number must come from the schedule. This is an instant hard failure in both tiers.
...
PICK ORDER IN THE OUTPUT ARRAY MATTERS: edge picks first in conviction order, then board picks grouped by sport.
...
{"picks":[
  {"tier":"edge","matchup":"Away @ Home","league":"NBA","type":"spread","when":"Today 7:30 PM ET","pick":"Team -4.5","odds":"-110","edge":"Specific named edge with source. Why the market hasn't adjusted. The bust case.","confidence":"medium"},
  {"tier":"board","matchup":"Away @ Home","league":"NHL","type":"moneyline","when":"Today 8:00 PM ET","pick":"Team ML","odds":"-135","edge":"One paragraph: the angle and the price, copied from the lines list.","confidence":"low"}
]}
...
A day with zero picks is not a failure. It's discipline.`;
  ...
  try {
    ...
    let finalPicks = raw.map(p => {
      ...
      const tier = String(p.tier||'').toLowerCase() === 'board' ? 'board' : 'edge';
      return { ...p, type: validTypes.has(t) ? t : 'spread', tier };
    });
    console.log('[picks] stage 3/3: finalized ' + finalPicks.length + ' picks');
    finalPicks.forEach((p, i) => {
      console.log('[picks] candidate ' + (i+1) + ' [' + p.tier + ']: ' + (p.league||'?') + ' / ' + (p.pick||'?') + ' (conf: ' + (p.confidence||'?') + ', edge len: ' + (p.edge||'').length + ')');
    });
...
You MUST return exactly 1 pick. Not zero. Pick the candidate with the most concrete supporting facts from the briefings above.`;
    ...
    // ─── TIER SPLIT ────────────────────────────────────────────────────────────
    // The strict discipline filter exists to protect the headline record, so it
    // applies to the EDGE tier only. Board picks are deliberately lighter-bar,
    // honestly-labeled volume — they get a sanity gate, not the full filter.
    const edgeCandidates  = finalPicks.filter(p => p.tier !== 'board');
    let   boardCandidates = hasOdds ? finalPicks.filter(p => p.tier === 'board') : [];
    if (!hasOdds && finalPicks.some(p => p.tier === 'board')) {
      console.log('[picks] dropping all board picks — no live lines this run (board requires real prices)');
    }

    // Strict pass: drop any EDGE pick that smells weak.
    const scored = edgeCandidates.map(p => ({ pick: p, ...scoreCandidate(p) }));
    let strictPicks = scored
      .filter(s => s.score >= 4 && !HEDGE_PHRASES.some(rx => rx.test(String(s.pick.edge||''))))
      .map(s => s.pick);

    // FALLBACK: if strict filter killed everything, take the highest-scoring
    // single candidate Claude returned. This guarantees at least one pick goes
    // up on the morning post. Empty slate is reserved for "Claude returned []".
    if (strictPicks.length === 0 && scored.length > 0) {
      const best = scored.slice().sort((a,b) => b.score - a.score)[0];
      console.log('[picks] strict filter zeroed all edge picks; falling back to best candidate (score ' + best.score + '): ' + best.pick.pick);
      strictPicks = [best.pick];
    }

    // Cap at 3 edge picks max. If Claude returned more, take the top 3 by score
    // (preserves Claude's intended ranking when scores tie).
    if (strictPicks.length > 3) {
      strictPicks = strictPicks
        .map(p => ({ pick: p, ...scoreCandidate(p) }))
        .sort((a,b) => b.score - a.score)
        .slice(0, 3)
        .map(s => s.pick);
    }

    // Log every drop reason for debugging when the morning slate looks off.
    scored.forEach(s => {
      const survived = strictPicks.includes(s.pick);
      console.log('[picks] EDGE ' + (survived ? 'KEEP' : 'DROP') + ' (score ' + s.score + ', ' + s.reasons.join(', ') + '): ' + s.pick.pick);
    });

    // ─── BOARD SANITY GATE ─────────────────────────────────────────────────────
    // Lighter bar, but not no bar: a board pick needs a stated rationale and an
    // honest confidence label. Hard line-existence enforcement happens after
    // odds matching below (board picks without a real snapshot are dropped).
    const BOARD_MAX_PER_SPORT = 3;
    const boardSportCount = {};
    boardCandidates = boardCandidates.filter(p => {
      const conf = String(p.confidence||'').toLowerCase();
      if (String(p.edge||'').trim().length < 40) {
        console.log('[picks] BOARD DROP (no rationale): ' + p.pick);
        return false;
      }
      // Board confidence is low or medium only — "high" belongs on the edge tier.
      if (conf !== 'low' && conf !== 'medium') p.confidence = 'medium';
      const lg = String(p.league||'other').toUpperCase();
      boardSportCount[lg] = (boardSportCount[lg]||0) + 1;
      if (boardSportCount[lg] > BOARD_MAX_PER_SPORT) {
        console.log('[picks] BOARD DROP (>' + BOARD_MAX_PER_SPORT + ' for ' + lg + '): ' + p.pick);
        return false;
      }
      // Never duplicate an edge pick's exact game + side on the board.
      const id = pickId(p);
      if (strictPicks.some(ep => pickId(ep) === id)) {
        console.log('[picks] BOARD DROP (duplicates edge pick): ' + p.pick);
        return false;
      }
      return true;
    });

    finalPicks = strictPicks.concat(boardCandidates);
    console.log('[picks] discipline filter: ' + beforeCount + ' → ' + strictPicks.length + ' edge + ' + boardCandidates.length + ' board');
    ...
    // ─── BOARD = REAL LINES ONLY ───────────────────────────────────────────────
    // The whole point of the board tier is "real sportsbook prices on every
    // pick." A board pick that couldn't be matched to a live bookmaker snapshot
    // would render as "no line snapshot" — exactly what the board must never
    // do — so it gets dropped here. Edge picks keep the old tolerance: research
    // edges (especially player props, which The Odds API base plan doesn't
    // price) are allowed to ship without a snapshot.
    const beforeBoardCheck = finalPicks.length;
    finalPicks = finalPicks.filter(p => {
      if (p.tier === 'board' && !(p.marketSnapshot && p.marketSnapshot.markets)) {
        console.log('[picks] BOARD DROP (no real line snapshot): ' + p.pick);
        return false;
      }
      return true;
    });
    if (finalPicks.length !== beforeBoardCheck) {
      console.log('[picks] board line check: ' + beforeBoardCheck + ' → ' + finalPicks.length);
    }
    ...
    try {
      if (existsSync('stories.json')) {
        ...
        prevPicks.forEach(p => {
          ...
          if (!ts) { fresh.push(p); return; } // unparseable — keep, don't blank the page
