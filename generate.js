...
YOUR JOB: PRODUCE 1–3 EDGE PICKS — THE BEST OF TODAY'S SLATE.
...
• Target: 2–4 picks per day. Spread them across the available sports when possible.
• Single-sport days are a RED FLAG that you're filtering too hard. If 3 sports have games today, your picks should ideally touch at least 2 of them. If you find yourself with all picks from one sport, force yourself to surface the best researched candidate from a second sport — even if it's medium-confidence.
• Player props are FIRST-CLASS picks, not consolation prizes. A clear stat-vs-defense mismatch IS an edge — you don't need breaking news to make a player prop a real pick.
• The first pick is the BEST PLAY of the day — your highest conviction.
• Picks 2-4 are independent — each must clear the bar on its own merits.

TIE-BREAKER WHEN CHOOSING THE BEST PICK:
...
DO NOT round up just to "have more picks." DO NOT pad to fill sport categories with garbage. But DO surface medium-confidence sport-diversified picks rather than returning a single-sport slate. A medium-confidence pick on a researched candidate is better than zero picks for that sport — empty days lose user trust.

If you genuinely cannot find ANY confirmed edge across the entire slate (truly rare — All-Star break with one game on TV), return your best researched candidate as 1 medium-confidence pick.

═══════════════════════════════════════════════════════════════
CONFIDENCE LABELS
═══════════════════════════════════════════════════════════════

  • "high"   = multiple supporting facts, line clearly hasn't adjusted, you'd bet 2 units
  • "medium" = one strong fact, line offers fair-to-slightly-good price, you'd bet 1 unit
  • DO NOT use "low" — if the pick is "low confidence," it's not a pick. Drop it.

If most picks would be "medium," that's correct. "High" should be rare.
...
• Generic narrative: "Team A is the better team and should win at home." Drop.
• Stats with no source: "they cover 67% of the time in this spot" without the research briefing backing it. Drop.
• Hedge language: "lean," "small edge," "could go either way," "feels like." If you wrote those words, you don't have a pick.
• Sport quotas: "I should add an NHL pick." No, you shouldn't, unless an NHL game has an edge.
• "Fair line, here's the side I prefer." This is the exact profile of a -110 coin flip and it is the reason records like 9-14 happen. DROP IT.
...
PICK ORDER IN THE OUTPUT ARRAY MATTERS: index [0] is your top conviction play, [1] second, [2] third.
...
{"picks":[{"matchup":"Away @ Home","league":"NBA","type":"spread","when":"Today 7:30 PM ET","pick":"Team -4.5","odds":"-110","edge":"Specific named edge with source. Why the market hasn't adjusted. The bust case.","confidence":"medium"}]}
...
A day with zero picks is not a failure. It's discipline.`;
  ...
  const res = await fetch('https://api.anthropic.com/v1/messages', {
    ...
    body: JSON.stringify({
      model:'claude-haiku-4-5-20251001',
      // Final output is at most 3 picks × ~250 tokens per pick of edge text
      // ≈ 800 tokens. 2000 leaves room for JSON overhead and verbose edges.
      max_tokens: 2000,
...
You MUST return exactly 1 pick. Not zero. Pick the candidate with the most concrete supporting facts from the briefings above.`;
    ...
    // Strict pass: drop anything that smells weak.
    const scored = finalPicks.map(p => ({ pick: p, ...scoreCandidate(p) }));
    ...
    if (strictPicks.length === 0 && scored.length > 0) {
      ...
      console.log('[picks] strict filter zeroed all picks; falling back to best candidate (score ' + best.score + '): ' + best.pick.pick);
      strictPicks = [best.pick];
    }

    // Cap at 3 picks max. If Claude returned more, take the top 3 by score
    ...
    scored.forEach(s => {
      ...
      console.log('[picks] ' + (survived ? 'KEEP' : 'DROP') + ' (score ' + s.score + ', ' + s.reasons.join(', ') + '): ' + s.pick.pick);
    });

    finalPicks = strictPicks;
...
async function main() {
  ...
  try {
    if (existsSync('stories.json')) {
      ...
      priorPicksCount = Array.isArray(prev.picks) ? prev.picks.length : 0;
    }
  ...
  if (shouldRunPicks) {
    ...
    // Off-cycle run: keep the picks already on disk so the site doesn't go blank.
    // BUT drop picks whose game time has already passed — otherwise yesterday's
    // (or last week's) pick gets copied forward forever and the site looks
    // frozen on stale content. Parse the "when" field the same way the front
    // end does to compute a scheduled epoch ms, then drop anything older than
    // 2 hours past tip-off (small grace period for late-finishing games).
    function parseWhenToTs(whenStr) {
      ...
      // The "when" was written when stories.json was first generated (this morning
      // or a prior day). We don't know exactly which day — but ANY pick where the
      // computed timestamp + 2h is in the past is stale, regardless of which
      // "today" it referred to. So compute timestamp using today's date; if it's
      // already past, treat as stale.
      const d = new Date();
      d.setDate(d.getDate() + dayOffset);
    ...
    try {
      if (existsSync('stories.json')) {
        ...
        existingPicks = prevPicks.filter(p => {
          const ts = (typeof p._scheduledTs === 'number') ? p._scheduledTs : parseWhenToTs(p.when);
          if (!ts) {
            // Couldn't parse — assume it's recent and keep it. Better to err on the
            // side of showing a pick than blanking the page.
            return true;
          }
          if (nowMs > ts + TWO_HOURS) {
            // Don't just drop the pick — capture it so we can grade it from the
            // game's final score and persist the result. Otherwise picks that
            // finish between page-loads disappear before being recorded.
            console.log('[picks] queueing for grading (game finished): ' + p.pick + ' @ ' + p.when);
            finishedPicksToGrade.push(p);
            return false;
          }
          return true;
        });
    ...
    clustered.picks = existingPicks;
    console.log('Skipped picks generation (off-cycle run); preserved ' + existingPicks.length + ' non-stale existing picks');

    // ─── SERVER-SIDE PICK GRADING ──────────────────────────────────────────────
    // For each finished pick, fetch the game's final score from ESPN and determine
    // W/L/Push. Persist results to pick_history.json so the front-end has a
    // canonical record (instead of relying on every visitor's localStorage, which
    // misses grades that happen while no one is on the site).
    if (finishedPicksToGrade.length > 0) {
      try {
        console.log('[grading] checking final scores for ' + finishedPicksToGrade.length + ' finished picks...');
        const graded = await gradeFinishedPicks(finishedPicksToGrade);
        if (graded.length > 0) {
          mergePickHistory(graded);
          console.log('[grading] persisted ' + graded.length + ' graded picks to pick_history.json');
        } else {
          console.log('[grading] no picks could be graded this run (will retry next cycle)');
          // Critical: picks that couldn't be graded yet (e.g., score not yet
          // posted on ESPN) get put BACK into existingPicks so the next run
          // tries again. Otherwise a pick that finishes at 11pm but ESPN
          // doesn't show as FINAL until 12am gets silently lost.
          const ungradeable = finishedPicksToGrade.filter(p => !graded.find(g => g.id === pickId(p)));
          clustered.picks = clustered.picks.concat(ungradeable);
          console.log('[grading] re-queued ' + ungradeable.length + ' picks for next run');
        }
      } catch (gradeErr) {
        // Grading failure shouldn't kill the whole workflow — re-queue and try
        // next time. The pick stays on the site but won't display as graded
        // until ESPN data is available.
        console.error('[grading] FATAL error during grading, re-queueing all picks:', gradeErr.message);
        clustered.picks = clustered.picks.concat(finishedPicksToGrade);
      }
    }
  }
