...
async function generatePicks() {
  ...
  const prompt = `You are a sharp sports bettor. Today is ${today}. Here are the real upcoming games pulled from ESPN right now:

${gameList}

Analyze these games and identify 3-4 of the best value picks. Use your knowledge of these teams — their recent form, injury history, home/away splits, schedule spots, back-to-back situations, and matchup edges. Be specific about WHY each pick has value.
  ...
  "picks": [
    {
      ...
      "league": "NBA",
      "when": "Wed Apr 23 7:30 PM ET",
      "pick": "Away Team +4.5",
      "odds": "-110",
      "edge": "Specific edge: e.g. Home team on 2nd night of back-to-back, Away team 8-2 ATS as road underdog this season, key home player dealing with knee issue",
      "confidence": "high"
    }
  ]
}

Rules: only use games from the list above. confidence = high, medium, or low. Always return 3-4 picks — pick the best spots available even if edges are not perfect. Never return empty picks.`;
