...
async function generatePicks(headlines) {
  ...
  const prompt = `You are a sharp sports bettor. Today is ${new Date().toLocaleDateString('en-US',{weekday:'long',month:'long',day:'numeric',year:'numeric'})}. Based on these sports business headlines, identify 3-4 upcoming real games this week with genuine betting value. Look for edges from contract disputes, roster moves, travel schedules, financial pressure, and playoff stakes.
...
{
  "picks": [
    { "matchup": "Team A vs Team B", "pick": "Team A -3", "odds": "-110", "edge": "reason based on news", "confidence": "high" }
  ]
}

Rules: only include real games you know are scheduled this week. confidence must be high, medium, or low. 3-4 picks only. Return nothing but the JSON object.`;
