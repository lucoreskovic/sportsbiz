...
async function fetchHighlights() {
  const LEAGUES = [
    ...
    { name: 'MLB',  channelId: 'UCzWQYUVCpZqtN93H8RR44Qw' },
    { name: 'NHL',  channelId: 'UCqFCMJ17JCBqjR8_9yx9wqg' },
    { name: 'WNBA', channelId: 'UCvFZOh6gkFVVPbSNt9ohEiA' },
    { name: 'MLS',  channelId: 'UCiWLfSweyRNmLpgEHekhoAg' },
  ];
  const results = [];
  for (const lg of LEAGUES) {
    try {
      const ytUrl = encodeURIComponent(`https://www.youtube.com/feeds/videos.xml?channel_id=${lg.channelId}`);
      const res = await fetch(`https://api.rss2json.com/v1/api.json?rss_url=${ytUrl}&count=1`, {
      ...
      if (data.status === 'ok' && data.items && data.items.length > 0) {
        ...
        if (videoId) {
          ...
          results.push({
            league: lg.name,
            title: item.title || lg.name + ' Highlights',
            videoId: videoId.split('&')[0],
            thumbnail: `https://img.youtube.com/vi/${videoId.split('&')[0]}/mqdefault.jpg`,
            timeAgo: h < 24 ? h + 'h ago' : Math.round(h/24) + 'd ago',
            isFallback: false,
          });
          console.log(`Got ${lg.name} video: ${videoId.split('&')[0]} — ${item.title.slice(0,40)}`);
        } else {
          console.warn(`No videoId in RSS2JSON for ${lg.name}`);
        }
      } else {
        console.warn(`RSS2JSON failed for ${lg.name}: ${data.status}`);
      }
    } catch(e) {
      console.warn(`Highlights error ${lg.name}: ${e.message}`);
    }
  }
  console.log(`Highlights fetched: ${results.length}/6`);
  return results;
}
