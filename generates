...
YOUR JOB IS TO PASS, NOT TO PICK.

Sportsbooks operate at razor-thin margins. Most days have no genuine edge. Real handicappers pass on the majority of games. Returning ZERO picks on a quiet day is a CORRECT answer — better than returning weak picks that drag down the long-term hit rate.
...
Required minimum thresholds — drop the pick if not met:
  - Spreads: model line vs market line gap ≥ 1.5 points
  - Totals:  model total vs market total gap ≥ 1.5 points
  - Moneylines: model probability vs market implied probability gap ≥ 5 percentage points
  - Player props: player's recent-form average vs line gap ≥ 20% of the line
...
• MAX 3 picks total. Probably fewer.
...
A day with zero picks is not a failure. It's discipline.`;
  ...
  try {
    ...
    // ─── DISCIPLINE FILTER ─────────────────────────────────────────────────────
    // Defense in depth — even with a tight prompt, drop weak picks server-side:
    //   - Confidence "low" → drop (prompt forbids low; this is belt-and-suspenders)
    //   - Edge text too short to contain real reasoning → drop
    //   - Edge text without numbers → drop (can't quantify edge without numbers)
    //   - Edge text without market-vs-model comparison → drop (when odds were available)
    ...
    finalPicks = finalPicks.filter(p => {
      ...
      if (edge.length < 100) return false; // bumped from 80 — quantified math takes space
      if (!/\d/.test(edge)) return false;  // must contain numbers
      if (hadOdds) {
        // When we had real lines available, the edge field MUST reference comparison
        // to the actual market. Look for words that indicate model-vs-market math.
        const hasMarketCompare = /\b(market|line|book|gap|fair|model)\b/i.test(edge);
        if (!hasMarketCompare) return false;
      }
      return true;
    });
    console.log('[picks] after discipline filter: ' + finalPicks.length + ' picks remain');
